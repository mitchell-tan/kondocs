...
Add a step Parse JSON, and decide which fields you want to sync from Kondo to Attio, then add them

Use these path names to get the right data from Kondo.
...
Add Alias names for each of the fields

### Step 4: Find record

Find a matching record in Attio to update. 

Filter -\> LinkedIn -\> contains -\> {kondo LinkedIn Handle}

Note that for this to work well, your records in Attio need to already have a Linkedin URL.
