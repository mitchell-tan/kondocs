...
### Create a new Apps Script in Google Sheets
...
function openSuccessPrompt(e) {
  const html = HtmlService.createHtmlOutput(
    ...
    '        <p class="text-6xl h-6 -translate-y-8">🎉</p>' +
    '        <p>&nbsp;</p>' +
    '        <p>Your webhook is now active and ready to use.</p>' +
    '        <p>&nbsp;</p>' +
    '        <p>To connect, go to the toolbar and select:<p>' +
    '        <p><strong>Kondo Webhook</strong> → <strong>Connect to Kondo</strong></p>' +
    '        <p>&nbsp;</p>' +
...
### Paste the webhook destination in Kondo
